...
decision: "LLM 대화 레이어와 결정론적인 설계 데이터 모델(존·집기·치수)을 분리했습니다. 대화는 의도와 전략을 받고, 평면은 검증 가능한 구조화 데이터로 두어 2D·3D 렌더와 DXF 익스포트가 같은 데이터를 보고 나오게 했습니다. 전략도 하나로 단정하지 않고 여러 안을 근거와 함께 제시해 사람이 고르게 했습니다."
...
decisionLog:
  - why: "대화(생성)와 설계 데이터(상태)를 분리해, LLM이 다시 그려도 평면 데이터의 원본을 한 곳으로 유지하고 존·집기 수를 검증할 수 있게 했습니다."
