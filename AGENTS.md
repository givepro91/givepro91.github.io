...
- **폰트 토큰은 `src/styles/global.css` 의 `:root` 하나뿐 (2026-09-07 학습).** `--mono`·`--sans` 를 페이지·컴포넌트에서 다시 정의하지 말고 그대로 참조한다. `--mono` 의 JetBrains Mono 에는 **한글 글리프가 없어** 스택 뒤쪽 한글 폴백(`Pretendard`·`Apple SD Gothic Neo`·`Malgun Gothic`)이 받아주는데, 지역 재정의는 그걸 잃는다. 그러면 **Windows 에서만** generic `monospace`(=Courier New) → 폰트링크 굴림체로 떨어져 글자가 뭉개진다 — **macOS 에서는 시스템이 Apple SD Gothic Neo 로 이어받아 절대 재현되지 않으니 맥 확인만으로 닫지 말 것.** 한글 문장에는 애초에 `--mono` 를 쓰지 않는다(공백까지 고정폭이라 어절이 벌어진다). 사례: `portfolio/print.astro` 의 `.deck` 지역 재정의 — commit `23ff19d` 로 삭제.
